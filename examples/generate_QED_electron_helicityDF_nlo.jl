...
const setname = "QED_electron_helicityDF_nlo"
...
info_str = """
SetDesc:         "QED helicity distribution functions of electron, evolved from NLO initial conditions."
Authors:         Lin Youjack
Reference:       N/A
Format:          lhagrid1
DataVersion:     1
...
dat_head_str = dat_head_str[1:end-1] * "\n"
dat_head_str *= "11 1100\n"

# distribution functions ---------------------------------------------------------------------------

ĝv_mμ = evolve_nonsinglet_ĝ_from_μ₀²_to_μ²(αEMe, s-> ĝll₀(αEMe,s),                me^2, mμ^2, 1)
ĝs_mμ = evolve_singlet_ĝ_from_μ₀²_to_μ²(   αEMe, s->[ĝll₀(αEMe,s); ĝγl₀(αEMe,s)], me^2, mμ^2, 1)

# μ > mμ
ĝvμ²(μ²) = evolve_nonsinglet_ĝ_from_μ₀²_to_μ²(αEMμ, ĝv_mμ, mμ^2, μ², 2)
ĝsμ²(μ²) = evolve_singlet_ĝ_from_μ₀²_to_μ²(   αEMμ, ĝs_mμ, mμ^2, μ², 2)
ĝeμ²(μ²) = s -> ( ĝsμ²(μ²)(s)[1] + ĝvμ²(μ²)(s) )/2
ĝēμ²(μ²) = s -> ( ĝsμ²(μ²)(s)[1] - ĝvμ²(μ²)(s) )/2
ĝγμ²(μ²) = s ->   ĝsμ²(μ²)(s)[2]
...
open(joinpath(folder, "$(setname)_0000.dat"), "w") do dat_file
    ...
    for x in x_grid
        for Q in Q_grid
            write(dat_file,
                @sprintf("%.5E", (1-x) * mellin_inv(ĝeμ²(Q^2))(1-x)         ) * " " *
                @sprintf("%.5E", 1 - integrate_distribution(ĝeμ²(Q^2), 1-x) ) * "\n" )
        end
    end
