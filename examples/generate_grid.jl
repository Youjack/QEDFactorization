...
# only e and μ
# the smallest value of Q is larger than mμ
get_αEM(μ²) = evolve_αEM_from_μ₀²_to_μ²(αEMμ, mμ^2, μ², 2)
...
"""
    generate_grid(particle, type, order)

- `particle`: `electron`, `muon`
- `type`: `DF`, `helicityDF`, `FF`
- `order` of initial conditions: `lo`, `nlo`
"""
function (@main)(args)

if length(args) != 3
    error("Usage: julia generate_grid.jl <particle> <type> <order>")
end
particle = args[1]
type = args[2]
order = args[3]
...
if type == "DF"
    _P̂s = P̂s
    f̂v_ml = N ->   f̂ll₀(αEMl, N)
    f̂s_ml = N -> [ f̂ll₀(αEMl, N); f̂γl₀(αEMl, N) ]
elseif type == "helicityDF"
    _P̂s = ΔP̂s
    f̂v_ml = N ->   ĝll₀(αEMl, N)
    f̂s_ml = N -> [ ĝll₀(αEMl, N); ĝγl₀(αEMl, N) ]
elseif type == "FF"
    _P̂s = ℙ̂s
    f̂v_ml = N ->   D̂ll₀(αEMl, N)
    f̂s_ml = N -> [ D̂ll₀(αEMl, N); D̂lγ₀(αEMl, N) ]
else
...
if order == "lo"
    ...
    f̂s_ml = N -> [ 1.0; 0.0 ]
elseif order == "nlo"
    # use already defined f̂v_ml, f̂s_ml
else
    error("Unsupported order: $order")
end

if particle == "electron"
    f̂v_mμ = evolve_nonsinglet_from_μ₀²_to_μ²(P̂ll, αEMe, f̂v_ml, me^2, mμ^2, 1 )
    f̂s_mμ = evolve_singlet_from_μ₀²_to_μ²(   _P̂s, αEMe, f̂s_ml, me^2, mμ^2, 1 )
else
    f̂v_mμ = f̂v_ml
    f̂s_mμ = f̂s_ml
end

f̂vμ²(μ²) = evolve_nonsinglet_from_μ₀²_to_μ²(P̂ll, αEMμ, f̂v_mμ, mμ^2, μ², 2)
f̂sμ²(μ²) = evolve_singlet_from_μ₀²_to_μ²(   _P̂s, αEMμ, f̂s_mμ, mμ^2, μ², 2)
f̂lμ²(μ²) = N -> ( f̂sμ²(μ²)(N)[1] + f̂vμ²(μ²)(N) )/2
# f̂l̄μ²(μ²) = N -> ( f̂sμ²(μ²)(N)[1] - f̂vμ²(μ²)(N) )/2
# f̂γμ²(μ²) = N ->   f̂sμ²(μ²)(N)[2]

# strings ------------------------------------------------------------------------------------------

setname = "QED_$(particle)_$(type)_$(order)"

info_str = """
SetDesc:         "QED $type of $particle, evolved from $order initial conditions."
Authors:         Lin Youjack
Reference:       N/A
Format:          lhagrid1
DataVersion:     1
...
for x in x_grid
    dat_str *= @sprintf("%.5E ", x)
end
dat_str = dat_str[1:end-1] * "\n"
for Q in Q_grid
    dat_str *= @sprintf("%.5E ", Q)
end
dat_str = dat_str[1:end-1] * "\n"

# distribution functions ---------------------------------------------------------------------------

print("Generating $setname ...")

dat_str *= "$id $(100id)\n"
for x in x_grid
    for Q in Q_grid
        dat_str *=
            @sprintf("%.5E", (1-x) * mellin_inv(f̂lμ²(Q^2))(1-x)         ) * " " *
            @sprintf("%.5E", 1 - integrate_distribution(f̂lμ²(Q^2), 1-x) ) * "\n"
    end
end
dat_str *= "---\n"
